...
function UploadIcon() {
  return (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none"
      className="text-ink-soft/70" aria-hidden="true">
      <path d="M12 16V4M12 4l-4 4M12 4l4 4" stroke="currentColor"
  ...
  return (
    <div className="bg-white/60 border border-paper-line rounded-sm p-6 shadow-sm">
      ...
      <p className="text-sm text-ink-soft mb-4">
        We'll check it against the issuing board's record and scan for signs of
        tampering.
      </p>
      ...
      <label
        ...
        onDrop={handleDrop}
        className={`flex flex-col items-center justify-center gap-2 h-44
          border-2 border-dashed rounded-sm cursor-pointer transition-colors
          ${isDragging
            ? "border-steel bg-steel/5"
            : "border-paper-line hover:border-steel/60"
          }`}
      >
        <UploadIcon />
        <span className="text-sm text-ink-soft text-center px-4">
          Drag a certificate here, or{" "}
          <span className="text-steel underline">browse your files</span>
        </span>
      ...
      {fileName && !validationError && (
        ...
        >
          ...
          {preview ? (
            ...
            <div className="flex items-center gap-2 text-sm text-ink bg-paper
              rounded-sm border border-paper-line px-3 py-2">
              <PdfIcon />
              {fileName}
            </div>
          )}
